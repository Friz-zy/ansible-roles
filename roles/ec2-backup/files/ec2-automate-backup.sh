...
get_EBS_List() {
  case $selection_method in
    volumeid)
      ...
      ebs_backup_list=$(aws ec2 describe-volumes --region $region --volume-ids $volumeid --output text --query 'Volumes[*].VolumeId')
      ;;
    tag)
      if [[ -z $tag ]]; then
      ...
      ebs_backup_list=$(aws ec2 describe-volumes --region $region --filters Name=tag:$tag --output text --query 'Volumes[*].VolumeId')
      ;;
    running)
      ebs_backup_list=$(aws ec2 describe-instances --region $region --filter "Name=instance-state-name,Values=running" --output text --query 'Reservations[*].Instances[*].BlockDeviceMappings[*].Ebs.VolumeId')
      ;;
  ...
  #list of all volumes of running instances
  #ebs_backup_list=$(aws ec2 describe-instances --region $region --filter "Name=instance-state-name,Values=running" --output text --query 'Reservations[*].Instances[*].BlockDeviceMappings[*].Ebs.VolumeId')
...
create_EBS_Snapshot_Tags() {
  ...
  if $name_tag_create; then
    InstanceId=$(aws ec2 describe-volumes --volume-ids ${ebs_selected} | python -c 'import sys, json; print json.load(sys.stdin)["Volumes"][0]["Attachments"][0]["InstanceId"]')
    Device=$(aws ec2 describe-volumes --volume-ids ${ebs_selected} | python -c 'import sys, json; print json.load(sys.stdin)["Volumes"][0]["Attachments"][0]["Device"]')
    snapshot_tags="Key=Name,Value='$InstanceId=$Device=$current_date' $snapshot_tags"
  fi
  ...
  if [[ -n $snapshot_tags ]]; then
    ...
    for tag in $snapshot_tags;do
      echo $tag
      aws ec2 create-tags --resources $ec2_snapshot_resource_id --region $region --tags $tag --output text
    done
  fi
}
...
purge_EBS_Snapshots() {
  ...
  snapshot_purge_allowed=$(aws ec2 describe-snapshots --region $region --filters Name=tag:PurgeAllow,Values=true Name=tag:Volume,Values=$ebs_selected --output text --query 'Snapshots[*].SnapshotId')
