...
function mysql_batch_query {
  ...
  sudo $MYSQL_BIN $opts $MYSQL_OPTS --batch -e "$query" 2>/dev/null
}
...
case $1 in
  "uptime")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f2 -d':' | cut -f1 -d'T' | sed -e 's/^\s*//'
    ;;

  "threads")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f3 -d':' | cut -f1 -d'Q' | sed -e 's/^\s*//'
    ;;

  "slowqueries")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f5 -d':' | cut -f1 -d'O' | sed -e 's/^\s*//'
    ;;

  "questions")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f4 -d':' | cut -f1 -d'S' | sed -e 's/^\s*//'
    ;;

  "qps")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f9 -d':' | sed -e 's/^\s*//'
    ;;

  "ping")
    sudo $MYSQLADMIN_BIN $mysql_opts status | grep alive | wc -l
    ;;

  "opens")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f6 -d':' | cut -f1 -d'F' | sed -e 's/^\s*//'
    ;;

  "opentables")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f8 -d':' | cut -f1 -d'Q' | sed -e 's/^\s*//'
    ;;

  "flushtables")
    sudo $MYSQLADMIN_BIN $mysql_opts status | cut -f7 -d':' | cut -f1 -d'O' | sed -e 's/^\s*//'
    ;;
  ...
  "replication" | "slave")
    if [ ! -e $MYCNF_PATH ]; then
      test -n "$slave_host" && slave_opts=" -h $slave_host "
      test -n "$slave_user" && slave_opts=" -u $slave_user "
      test -n "$slave_password" && slave_opts=" --password=$slave_password "
    else
